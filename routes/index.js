...
//  });
    ...
    var url;
    if (!req.query.url)
        //res.status(500).json({"error":"please supply a url query parameter"});
        ...
        webshot(url, options, function (err, renderStream) {
            ...
            var key = encodeURIComponent(url) + '.png';
            ...
            renderStream.on('end', function () {
                ...
                pushImage(key, imageBuffer, function(err, result) {
                    if (err)
                        res.status(500).json({"error":err});
                    else
                        res.render('index', { title: 'Express', pageContent: result });
                });
            });
        });
    }
...
var pushImage = function(key, imageBuffer, callback) {
    s3.createBucket(function() {
        var params = {Bucket: process.env.BUCKETEER_BUCKET_NAME, Key: key, Body: imageBuffer};
        ...
        s3.putObject(params, function(err) {
            if (err) {
                ...
                s3.getSignedUrl('getObject', {Bucket: process.env.BUCKETEER_BUCKET_NAME, Key: key}, function(err, url) {
                    callback(null, url);
                });
            }
        });
